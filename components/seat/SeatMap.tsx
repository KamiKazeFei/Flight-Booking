...
import { addSeat, removeSeat, type Seat } from "@/lib/redux/slices/bookingSlice"
...
const generateSeats = (rows: number, cols: number, offset = 0, flightId: string, flightType: string): Seat[][] => {
  ...
  for (let row = 1; row <= rows; row++) {
    ...
    // Make some seats unavailable consistently based on row and flight
    ...
    for (let col = 0; col < cols; col++) {
      ...
      currentRow.push({
        ...
        number: seatNumber,
        type: unavailableSeats.has(col) ? "unavailable" : type,
        price,
        flightType: flightType as "outbound" | "return",
      })
    }
...
export function SeatMap({ flightId, flightType }: SeatMapProps) {
  ...
  // Filter seats for the current flight type
  const currentFlightSeats = Object.entries(selectedSeats).filter(([_, seat]) => seat.flightType === flightType)

  const handleSeatSelect = (seat: Seat) => {
    if (seat.type === "unavailable") return

    // Check if this seat is already selected
    const existingPassengerEntry = Object.entries(selectedSeats).find(([_, s]) => s.id === seat.id)

    if (existingPassengerEntry) {
      // If clicking on a seat that's already selected, deselect it
      dispatch(removeSeat(existingPassengerEntry[0]))

      // If we're deselecting the current passenger's seat, don't change the current passenger
      const passengerKey = `passenger-${currentPassenger}-${flightType}`
      if (existingPassengerEntry[0] === passengerKey) {
        // Do nothing to currentPassenger
      } else {
        // If we're deselecting a different passenger's seat, set current passenger to that one
        const passengerNumber = Number.parseInt(existingPassengerEntry[0].split("-")[1])
      ...
      // Check if current passenger already has a seat for this flight
      const currentPassengerSeatEntry = Object.entries(selectedSeats).find(([key, s]) => key === passengerKey)

      if (currentPassengerSeatEntry) {
        // Replace the current passenger's seat
        dispatch(removeSeat(passengerKey))
      }

      // Add the new seat
      dispatch(addSeat({ passengerId: passengerKey, seat }))

      // Move to next passenger if there are more
      if (currentPassenger < passengers) {
        const nextPassengerKey = `passenger-${currentPassenger + 1}-${flightType}`
        const nextPassengerHasSeat = Object.keys(selectedSeats).includes(nextPassengerKey)

        if (!nextPassengerHasSeat) {
  ...
  const getSeatColor = (seat: Seat) => {
    if (seat.type === "unavailable") return "bg-gray-300 text-gray-500 cursor-not-allowed opacity-50"
  ...
  const getTotalSeatPrice = () => {
    return currentFlightSeats.reduce((total, [_, seat]) => total + seat.price, 0)
  }
        ...
        <CardHeader>
          <CardTitle className="flex justify-between items-center">
            ...
            <div className="text-sm font-normal">
              Selected: {currentFlightSeats.length}/{passengers}
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 border-t pt-4">
            ...
            {currentFlightSeats.length > 0 ? (
              <div className="space-y-2">
                {currentFlightSeats.map(([passengerId, seat]) => (
                  <div key={passengerId} className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={getSeatColor(seat)}>
                        {seat.number}
                      </Badge>
                      <span>
                        Passenger {passengerId.split("-")[1]} -{" "}
